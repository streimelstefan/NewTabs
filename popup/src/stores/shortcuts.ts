...
export const useShortcutStore = defineStore("shortcut", {
  ...
  actions: {
    ...
    getScWithUrl(url: string) {
      return new Promise<ShortCut | undefined>((res, rej) => {
        console.log(Object.keys(this.shortcuts));
        Object.keys(this.shortcuts).forEach((sc) => {
          console.log(
            `${this.shortcuts[sc].url} === ${url} -> ${
              this.shortcuts[sc].url === url
            }`
          );
          if (this.shortcuts[sc].url === url) {
            const savedSc = this.shortcuts[sc];
            const reSc: ShortCut = {
              name: savedSc.name,
              shortcut: sc,
              url: url,
