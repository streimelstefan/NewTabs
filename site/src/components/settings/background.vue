...
<template>
    <div class="h-full">
        ...
        <div class="grid grid-cols-2 gap-x-10 mt-5 h-1/3">
            <form class="flex flex-col items-center" @submit.prevent="">
                <h2 class="w-full text-center text-lg text-white">
                    Get a random image
                </h2>
                <div
                    class="w-2/3 bg-opacity-70 backdrop-opacity-80 text-white bg-gray-900 rounded-md p-3"
                >
                    ...
                    <NtButton submit class="float-right" @click="getNewImage()"
                        >Get Image</NtButton
                    >
                </div>
            </form>
